...
An event is `Global-South` (and earns the 🌍 marker) iff **either**:
- it is **hosted** in a country that appears in this list (use the event's `country`
  field, parsed from its official page's location), **or**
- it explicitly **serves or prioritises** Global-South participants — a regional focus
  on LMICs, travel bursaries, or fee waivers stated on the official page.

An event hosted in an upper-middle- or high-income country with no LMIC-serving mandate
is **not** `Global-South`, even if attendees are international.
