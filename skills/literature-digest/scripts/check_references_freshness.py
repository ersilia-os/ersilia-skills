...
etc.) are supposed to be refreshed periodically — by default every 90 days — so
that the topic taxonomy, author lists, Hub priors, and journal tiers track what
Ersilia is actually doing.

This script reads `references/_state.json`, compares `next_refresh_due` to today,
and:
...
- prints a `DUE` line on stdout naming `last_refresh_date` and `next_refresh_due`
  and exits 0 when a refresh is due (the skill decides whether to block, warn,
  or prompt — exit code is non-fatal so the digest can still run),
- prints a `WARNING` on stderr and exits 1 if the state file is missing or
  malformed.

The script does **not** perform the refresh itself. It only flags the state.
The refresh procedure is documented in `references/refresh-procedure.md`.
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not last or not nxt:
        warn(f"state file is missing last_refresh_date or next_refresh_due")
        return 1
