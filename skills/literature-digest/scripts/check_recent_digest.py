"""Pre-flight guard: refuse to generate a new digest if one already exists for the
last N days.

A file `digests/YY-MM-DD-digest.md` is considered "recent" iff its embedded date
...
# Match `YY-MM-DD-literature-digest.md` (the canonical format from output-template.md).
# Also accept the legacy `YY-MM-DD-digest.md` form so old digests still block.
...
def parse_filename_date(filename: str) -> date | None:
    ...
    mm = int(m.group("mm"))
    dd = int(m.group("dd"))
    # Two-digit year: anything ≤ 80 is 21st century (2000-2080), 81-99 is 20th.
