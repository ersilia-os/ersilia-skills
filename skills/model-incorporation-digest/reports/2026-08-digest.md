...
| Model | Title | Task | Authors |
|---|---|---|---|
| [`eos3f8h`](https://github.com/ersilia-os/eos3f8h) | Antimicrobial activity prediction from EU OpenScreen data | Activity prediction | `Ctibor Škuta et al. (11)` |
| [`eos3xhm`](https://github.com/ersilia-os/eos3xhm) | HADES Oral Drug-Likeness | Property calculation or prediction | `Narek Petrosyan et al. (8)` |

### Representation — 4 models

| Model | Title | Task | Authors |
|---|---|---|---|
| [`eos5g6m`](https://github.com/ersilia-os/eos5g6m) | GLACIER Molecular Embeddings | Featurization | `Emily Nguyen et al. (5)` |
| [`eos5mnx`](https://github.com/ersilia-os/eos5mnx) | SAND Shape-Aware Descriptor | Featurization | `Robin Winter et al. (3)` |
| [`eos6pj2`](https://github.com/ersilia-os/eos6pj2) | NaFM Natural Product Embeddings | Featurization | `Yuheng Ding et al. (11)` |
| [`eos19dk`](https://github.com/ersilia-os/eos19dk) | MolCompass Chemical Space Projection | Projection | `Sergey Sosnin` |

### Sampling — 4 models

| Model | Title | Task | Authors |
|---|---|---|---|
| [`eos55vx`](https://github.com/ersilia-os/eos55vx) | CoCoGraph Formula-Preserving Generation | Generation | `Manuel Ruiz-Botella et al. (3)` |
| [`eos6a1h`](https://github.com/ersilia-os/eos6a1h) | CoCoGraph Small-Fragment Inpainting | Generation | `Manuel Ruiz-Botella et al. (3)` |
| [`eos84nf`](https://github.com/ersilia-os/eos84nf) | GenMol Scaffold Decoration | Generation | `Seul Lee et al. (9)` |
| [`eos8zvb`](https://github.com/ersilia-os/eos8zvb) | PyMolGen Drug-Like Molecule Generation | Generation | `Bruno N. Falcone et al. (11)` |
...
Predicts activity against seven reference pathogens — *A. baumannii*, *C. albicans*, *E. coli*, *E. faecalis*, *K. pneumoniae*, *P. aeruginosa* and *S. aureus* — from single-point inhibition screens of the ~100,000-compound EU OpenScreen library, taken from the European Chemical Biology Database. The credit here is for the **screening data**, not the model: Škuta and colleagues produced the database, and Ersilia trained these classifiers on it with LazyQSAR v3, reporting a mean AUROC of 0.94 across 5-fold cross-validation.

Paper: [10.1093/nar/gkae904](https://doi.org/10.1093/nar/gkae904) · Authors' code: [github.com/ersilia-os/eu-openscreen-antimicrobial-tasks](https://github.com/ersilia-os/eu-openscreen-antimicrobial-tasks) · Run it: `ersilia fetch eu-openscreen-hts` · Licence: GPL-3.0-or-later

#### `eos3xhm` · HADES Oral Drug-Likeness

**Narek Petrosyan, Hovakim Zakaryan and 6 colleagues** — Denovo Sciences Inc (Yerevan) · Institute of Molecular Biology of NAS (Yerevan) · Journal of Chemical Information and Modeling 2026

Scores how closely a compound resembles an approved oral drug, with 0.63 as the authors' recommended cut-off. Averages probabilities from five tree and boosting classifiers over 298 features combining Mordred descriptors, ADMET-AI predictions and QED terms, trained on 1,177 approved oral drugs against 5,307 non-drugs from ChEMBL, ZINC and GDB. The authors show scores rising across clinical phases and falling for orally toxic and chemically implausible structures. The institutions in the byline were taken from the paper: OpenAlex mis-resolves them to a Slovak institute, to Denso and to a Qatari science park.

Paper: [10.1021/acs.jcim.5c02953](https://doi.org/10.1021/acs.jcim.5c02953) · Authors' code: [github.com/Narek-Petros-yan/HADES](https://github.com/Narek-Petros-yan/HADES) · Run it: `ersilia fetch hades-oral-druglikeness` · Licence: MIT
...
Paper: [10.48550/arXiv.2606.11382](https://doi.org/10.48550/arXiv.2606.11382) · Authors' code: [github.com/eemokey/glacier](https://github.com/eemokey/glacier) · Run it: `ersilia fetch glacier-embeddings` · Licence: MIT

#### `eos5mnx` · SAND Shape-Aware Descriptor

**Robin Winter, Julian Cremer and Djork-Arné Clevert** — Pfizer · ICML 2026

Turns a molecule's 2D structure into a fixed-length embedding that captures its 3D shape without generating any conformers, so cosine similarity between vectors approximates 3D shape overlap. Useful for rapid retrieval of shape-similar molecules in ligand-based virtual screening. A GINE graph encoder producing 512 dimensions, trained at Pfizer and presented at ICML 2026. Its Publication field is an OpenReview URL, which mints no DOI, so the author list could not be resolved automatically; the three names above were transcribed by hand from a third-party index and should be checked against the paper before being relied on.

Paper: [https://openreview.net/forum?id=pB6WAdnRDR](https://openreview.net/forum?id=pB6WAdnRDR) · Authors' code: [github.com/pfizer-opensource/SAND](https://github.com/pfizer-opensource/SAND) · Run it: `ersilia fetch sand-shape-descriptor` · Licence: Apache-2.0
...
Paper: [10.1038/s42256-026-01226-8](https://doi.org/10.1038/s42256-026-01226-8) · Authors' code: [github.com/TomAIDD/NaFM-Official](https://github.com/TomAIDD/NaFM-Official) · Run it: `ersilia fetch nafm-embeddings` · Licence: MIT
...
Paper: [10.1186/s13321-024-00888-z](https://doi.org/10.1186/s13321-024-00888-z) · Authors' code: [github.com/sergsb/molcomplib](https://github.com/sergsb/molcomplib) · Run it: `ersilia fetch molcompass` · Licence: MIT
...
Paper: [10.1038/s42256-026-01229-5](https://doi.org/10.1038/s42256-026-01229-5) · Authors' code: [doi.org/10.5281/zenodo.18940151](https://doi.org/10.5281/zenodo.18940151) · Run it: `ersilia fetch cocograph-formula` · Licence: MIT
...
Paper: [10.1038/s42256-026-01229-5](https://doi.org/10.1038/s42256-026-01229-5) · Authors' code: [doi.org/10.5281/zenodo.18940151](https://doi.org/10.5281/zenodo.18940151) · Run it: `ersilia fetch cocograph-small` · Licence: MIT
...
Paper: [10.48550/arXiv.2501.06158](https://doi.org/10.48550/arXiv.2501.06158) · Authors' code: [github.com/NVIDIA-Digital-Bio/genmol](https://github.com/NVIDIA-Digital-Bio/genmol) · Run it: `ersilia fetch genmol-scaffold-decoration` · Licence: Apache-2.0

#### `eos8zvb` · PyMolGen Drug-Like Molecule Generation

**Bruno N. Falcone, Jonathan D. Hirst and 9 colleagues** — University of Nottingham · University of Strathclyde · GlaxoSmithKline (Spain) · Journal of Chemical Information and Modeling 2026
...
Paper: [10.1021/acs.jcim.6c00689](https://doi.org/10.1021/acs.jcim.6c00689) · Authors' code: [github.com/HirstGroup/PyMolGen](https://github.com/HirstGroup/PyMolGen) · Run it: `ersilia fetch pymolgen` · Licence: MIT
