"""Upload a rendered model-incorporation digest to the canonical remote location.

Default destination: `ersilia-os/digests` repo, path
`models/YY-MM-DD-models-digest.md`. Uses the GitHub contents API via `gh` so no
`git clone` is needed.

Upload the **public** render — `render_report.py --public` — not the internal one. The
internal digest carries a metadata-defects section that lists repairs owed on models that
are already live; it is written for the team and does not belong on a public page. This
script cannot tell the two apart, so passing the wrong file publishes the wrong document.

The date in the filename is the **last day of the month being reported**, matching the
window-end convention the sibling digests use: August 2026 publishes as
`26-08-31-models-digest.md`.

After a successful upload, the script also updates the repo's `README.md` to add
the new digest under the `## Model incorporation digests` section, formatted as:

    - [YYYY-MM-DD](models/YY-MM-DD-models-digest.md)
...
Usage:
    python upload_digest.py --digest /tmp/26-08-31-models-digest.md
    python upload_digest.py --digest /tmp/26-08-31-models-digest.md \
                            --message "model incorporation digest for August 2026"
    python upload_digest.py --digest /tmp/X-models-digest.md --force
    python upload_digest.py --digest /tmp/X-models-digest.md --no-readme
...
FILENAME_RE = re.compile(
    r"^(?P<yy>\d{2})-(?P<mm>\d{2})-(?P<dd>\d{2})-models-digest\.md$"
)

# Heading inside the README under which model-incorporation-digest entries live.
README_SECTION_HEADING = "## Model incorporation digests"

# Existing list entries that look like one of our digests. Match the
# `- [YYYY-MM-DD](models/YY-MM-DD-models-digest.md)` shape.
README_ENTRY_RE = re.compile(
    r"^-\s*\[(?P<full>\d{4}-\d{2}-\d{2})\]\((?P<href>models/[^)]+)\)\s*$"
)
...
def update_readme_index(
    ...
    `models/26-08-31-models-digest.md`); `filename` is just the basename
...
def _insert_or_skip_readme_entry(
    ...
    Behaviour:
    - If the README already has a `- [YYYY-MM-DD](models/...)` line with the
      same date, return (text, False) — nothing to change.
    - Otherwise locate `## Model incorporation digests`, insert the new entry in
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--remote-dir", default="models",
                   help="Path inside the repo where model incorporation digests live.")
    ...
    commit_message = args.message or f"Add {local.name} (model incorporation digest)"
