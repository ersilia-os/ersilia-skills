...
`models/YY-MM-DD-models-digest.md`. Before publishing a new digest the skill runs this
script to detect any existing digest dated within the last N days (default 25). If one
exists, the skill **stops**: re-running would clobber recent published work.

The window is 25 days rather than the 7 the weekly digests use, because this digest is
monthly. It is deliberately shorter than a month so that a month published early and the
next one published late do not collide.
...
- If the remote `models/` folder does not exist yet (HTTP 404 from gh):
  print nothing and exit 0 — this is the first-ever run for this category.
...
Usage:
    python check_remote_digest.py [--repo ersilia-os/digests] [--path models]
                                  [--days 25] [--ref main]
"""
