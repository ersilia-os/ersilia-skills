...
> **Schema change, 2026-08-13.** The metric columns (Stars, Forks, Open Issues, Subscribers,
> Total Commits, Contributors, Contributor Names) were **removed** from this table, and
> `Status` changed from `singleSelect` to `multipleSelects`. The metric write-back step the
> skill used to run was dropped as a result — there is nothing left to write. Re-check with
> `list_tables_for_base` if those columns ever come back.

## Locating the table (verified 2026-08-13)

- Base: **Ersilia Content** — `app1iYv78K6xbHkmL`
- Table: **Repositories** — `tbluZtI3W9pseCSPH` (≈180 records)
...
| `Status` | **multipleSelects** | **human** | Flag empty (`missing_status`). Choices: Todo, In progress, Completed, Archived, Discontinued, Idle. Join multiple values with `", "` when dumping. |
...
There are **no metric columns** on this table any more (see the schema-change note above).
...
- `id` is the record's top-level `rec…` id from the MCP result — keep it so a finding can be
  traced back to the row a human needs to edit.
- `name` is required (the join key). Records with no `Name` are counted as `airtable_unnamed`
  and skipped by reconcile.
- `type` and `projects` are lists; emit `[]` when empty. `status` is a **string** — the field is
  `multipleSelects`, so join multiple values with `", "` (e.g. `"Completed, Archived"`).
...
`Visibility`=`fldXhoqkmBs6ZRhnn`, `Creation Date`=`fldBH2270474FY9XW`. (Re-confirm IDs with
...
- **`metric_drift`** — *vestigial.* The metric columns no longer exist on the table, so this is
  always empty. Ignore it.
